package com.movierecommendation.spark.als;
...
public class ModelFactory {
	...
	public static TrainedModel create(JavaRDD<Rating> trainingRdd, JavaRDD<Rating> testRdd, int rank, int iterationsNr) {
        ...
        JavaRDD<Tuple2<Object, Object>> testForPredict = testRdd
                .map(rating -> new Tuple2<>(rating.user(), rating.product()));

        MatrixFactorizationModel model = ALS.train(JavaRDD.toRDD(trainingRdd), rank, iterationsNr, 0.1);

        Double error = getError(testRdd, rank, iterationsNr, testForPredict, model);

        return new TrainedModel(error, model);
    }

    private static Double getError(JavaRDD<Rating> testRdd, int rank, int iterationsNr,
                                   JavaRDD<Tuple2<Object, Object>> testForPredict, MatrixFactorizationModel model) {
